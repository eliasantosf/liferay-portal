...
const ClaimsTable = () => {
	...
	const filterSearchAndStatusAndProduct = `${filterSearch} and ${policyERCByProduct} and ${filterProduct}`;
	...
	const setFilterSearch = () => {
		...
		if (filterStatusCheck.length && !searchInput) {
			return filterStatus;
		}
		...
		if (searchInput && policyERCByProduct.length) {
	...
	const getPolicyERCByProductName = async () => {
		...
		setPolicyERCByProduct(policyERCs);
	};
