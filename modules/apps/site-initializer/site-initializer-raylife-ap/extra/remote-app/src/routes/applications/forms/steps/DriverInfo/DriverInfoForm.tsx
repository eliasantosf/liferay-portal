...
const DriverInfoForm = ({
	...
	const accidentCitations = form.map((form) => {
		const list = form.accidentCitation || [];
	...
	const handleAddCitationClick = () => {
		...
		setOptions([...options, newAccidentCitationOptions]);
					...
					>
						<ClayInput
							...
							onChange={(event) => {
								...
								value =
									value === 'Choose an option' ? '' : value;
								handleChangeField('otherOcupation', value, id);
								setHasError({
									...hasError,
									otherOccupation:
										value === '' &&
							...
							required
							type="text"
							value={form[formNumber - 1].otherOcupation}
						/>
						...
						<ClayDropDownWithItems
							...
							trigger={
								<div className="d-flex select-millitary-affiliation text-neutral-10">
									<ClaySelect
										className="bg-neutral-0 border-neutral-5 cursor-pointer select-style text-paragraph-sm"
										disabled
										required
										value={
											form[formNumber - 1]
									...
									>
										<ClaySelect.Option
											className="cursor-pointer py-4 text-brand-primary text-center text-paragraph-sm"
											label={
												form[formNumber - 1]
													.millitaryAffiliation
											}
